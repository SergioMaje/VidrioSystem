...
import { useCrearCuentaPagoEmpresa, useEditarCuentaPagoEmpresa } from '@/hooks/useCuentasPagoEmpresa'
...
interface CuentaEmpresaFormDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  cuenta?: CuentaPagoEmpresa | null
}

export function CuentaEmpresaFormDialog({ open, onOpenChange, cuenta }: CuentaEmpresaFormDialogProps) {
  const crearCuenta = useCrearCuentaPagoEmpresa()
  const editarCuenta = useEditarCuentaPagoEmpresa()
  ...
  useEffect(() => {
    if (open) {
      if (cuenta) {
        ...
        reset({ alias: '', banco: '', tipo_cuenta: '', numero_cuenta: '', titular: '', orden: 0, activo: true })
      }
    }
  }, [open, cuenta, reset])
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">Menor primero al imprimir.</p>
            </div>
            <div className="flex items-center gap-2 pt-6">
              <input id="activo" type="checkbox" className="h-4 w-4" {...register('activo')} />
              <Label htmlFor="activo" className="font-normal">Mostrar en las cotizaciones</Label>
            </div>
          </div>
          <DialogFooter>
